...
class Conv2dLayer(nn.Module):
    def __init__(self,
                 in_channels,                    # 输入通道数.
                 out_channels,                   # 输出通道数
                 kernel_size,                    # 卷积核的宽度和高度。
                 bias            = True,         # 是否在激活函数之前应用偏置
                 activation      = 'linear',     # 激活: 'relu', 'lrelu', etc.
                 up              = 1,            # 整数上采样因子。
                 down            = 1,            # 整数下采样因子。
                 resample_filter = [1,3,3,1],    # 对激活重新采样时应用的低通滤波器。
                 conv_clamp      = None,         # 将输出规范到 +-X, None = disable clamping.
                 trainable       = True,         # 是否在训练中更新这一层的权重（训练时为true）
                 ):
        super().__init__()
        self.activation = activation
        self.up = up
...
class StyleConv(torch.nn.Module):
    def __init__(self,
        ...
        kernel_size     = 3,            # 卷积核大小
        up              = 1,            # 整数上采样因子
        use_noise       = True,         # 是否启用噪声输入
        activation      = 'lrelu',      # 激活函数: 'relu', 'lrelu', etc.
        resample_filter = [1,3,3,1],    # 对激活重新采样时应用的低通滤波器
        conv_clamp      = None,         # 将输出规范到 +-X, None = disable clamping.
...
class MappingNet(torch.nn.Module):
    def __init__(self,
        z_dim,                      # 输入潜在维度（Z）, 0 = no latent
        c_dim,                      # 条件标签维度（C）, 0 = no label
        w_dim,                      # 中间潜在维度（W）
        num_ws,                     # 要输出的中间延迟数, None = do not broadcast
        num_layers      = 8,        # 映射层数
        embed_features  = None,     # 标签嵌入维度, None = same as w_dim
        layer_features  = None,     # 映射图层中的中间要素数量, None = same as w_dim
        activation      = 'lrelu',  # 激活函数: 'relu', 'lrelu', etc
        lr_multiplier   = 0.01,     # 映射层学习速率
        w_avg_beta      = 0.995,    # 训练期间跟踪W移动平均值的衰减, None = do not track
    ):
        super().__init__()
        self.z_dim = z_dim
        self.c_dim = c_dim
